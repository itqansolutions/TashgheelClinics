...
const schema = z.object({
  ...
  message: "Provide either a user or a full name",
  path: ["fullName"]
});
...
export function DoctorsPage() {
  ...
  const openModal = (doctor?: Doctor) => {
    reset({
      ...
      fullName:    doctor?.fullName    ? String(doctor.fullName)    : '',
  ...
  return (
    <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 p-4">
              {(doctors as Doctor[]).map((doc) => (
                <div key={doc.id} className="border border-gray-100 rounded-xl p-4 hover:border-gray-200 hover:shadow-sm transition-all group">
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex items-center gap-3">
                      <div className="flex items-center justify-center w-11 h-11 rounded-xl bg-brand-50 text-brand-700 text-sm font-bold shrink-0">
                        {getInitials(doc.user?.fullName ?? doc.fullName ?? '??')}
                      </div>
                      <div>
                        <p className="text-sm font-semibold text-gray-900">Dr. {doc.user?.fullName ?? doc.fullName}</p>
                  ...
                  <p className="text-[11px] text-gray-400 truncate mb-3">{doc.user?.email || 'No system account'}</p>
                  <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    ...
                    {doc.isActive && (
                      <Button variant="ghost" size="xs"
                        ...
                        onClick={() => { if (confirm(`Deactivate Dr. ${doc.user?.fullName || doc.fullName}?`)) deactivateDoctor.mutate(doc.id); }}>
                        Deactivate
                      </Button>
                    )}
                  </div>
      ...
      <Modal open={modal.open} onClose={() => setModal({ open: false })}
        ...
        subtitle={modal.editing ? `Dr. ${modal.editing.user?.fullName || modal.editing.fullName}` : 'Link a Doctor-role user'} size="sm">
        <form onSubmit={handleSubmit(onSubmit)} noValidate className="space-y-4">
          {!modal.editing && (
            <div className="space-y-4">
              <Select label="User (Doctor role)" error={errors.userId?.message}
                options={doctorUsers?.map((u) => ({ value: u.id, label: u.fullName })) ?? []}
                placeholder="Select user (optional)" {...register('userId')} />
              
              {!watch('userId') && (
                <Input label="Or enter Full Name" required placeholder="Dr. Name..."
